...
moves = [tuple(map(int, input().split())) for _ in range(m)] # 각 년도의 이동 규칙 (방향, 칸수). 방향 -1 하기
medic = [(n-1, 0), (n-2, 0), (n-1, 1), (n-2, 1)] # 영양제 위치. 최초 좌하

for d, p in moves: # 년마다 이동
    d -= 1 # 방향, 칸수

    # 1. 영양제 이동
    for i in range(len(medic)):
        mr, mc = medic[i]
        mr = (mr + (mdr[d] * p)) % n
        mc = (mc + (mdc[d] * p)) % n
